...
## 2026-09-09-1 — 버튼 폰트를 GD 방식으로 통일 (✅ 폰트 교체 검증 통과 · ✅ 독립 재검증 일치 · 🐛 후속 3건 남음)
...
1. **`柶`(U+67F6) 결손이 미수정이다** — 결함 1번. CloseButton(결함 2번)은 위에서 닫혔다.
   ...
   (`50,HouseYut,윷놀이,Yutnori,柶戏,ユンノリ`)
2. **SwapText 폭 여유가 0이다** — `Swap 2` 기준 필요폭/배정폭이 `144/144px`다.
   현재는 오버플로가 없지만 두 자리(`Swap 10`)가 되면 168px가 필요해 즉시 초과한다.
3. **한국어 혼합 폰트의 굵기·크기가 튄다** — 한글은 DungGeunMo, 숫자·영문은
   PressStart2P로 렌더돼 `추가 스핀 25G (2회)`·`배속 x1` 같은 한 줄에서 차이가 보인다.
   기능 영향은 없으며 미적 다듬기 방향을 정해야 한다.
