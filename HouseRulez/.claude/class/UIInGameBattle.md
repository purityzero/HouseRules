# UIInGameBattle

연관: [[BattleUnit]], [[InGameScene]], [[UIInGameField]], `JudgeResult`, `WaveRecord`, `EnemyRecord`, `UnitGradeRecord`, `GameConfigTable`

## 2026-08-30-0 — 신설 (웨이브 전투)

### 개요
웨이브 한 판. 아군은 판정 소환 결과에서, 적은 `WaveTable`에서 만들어 서로 진격시킨다.
**최소 수직 슬라이스**다 — 레인(릴의 행) 안에서만 교전하고 레인 간 간섭은 없다.
전열/중열/후열의 역할 차이(GDD §FieldLayout)는 전투가 실제로 도는 걸 본 뒤에 얹는다.

파일: `Assets/Scripts/InGame/Battle/UIInGameBattle.cs`

### 계층 구조 (InGameScene.unity)
```
SafeRoot
└─ Battle             RectTransform 1400x320, anchoredPos (520, 20)   ← UIInGameBattle (GO 1703635131)
   └─ UnitRoot        RectTransform 1400x320                          (rt 1680718474)
      └─ UnitTemplate ★ 비활성 원본 → [[BattleUnit]]
```
`Field`와 같은 x(520)에 둔다 — 소환 표시가 그대로 전투 유닛으로 이어지는 것처럼 보여야 한다.

### x 좌표 배치
| 필드 | 값 | 의미 |
|---|---|---|
| `m_AllyStartX` | 0 | 아군 출발선 |
| `m_EnemySpawnX` | 900 | 적 등장선 |
| `m_HomeLineX` | -60 | 적이 이 왼쪽을 넘으면 본거지가 맞는다 |

레인 오프셋은 `UIInGameField`와 같은 규칙(`LANE_STEP_Y = 52`, `LANE_STEP_X = 30`) — 소환 화면에서
전투 화면으로 넘어갈 때 유닛이 튀지 않도록 두 곳의 값을 맞춰뒀다.
★ 두 클래스에 같은 숫자가 있다. 한쪽만 고치면 어긋나므로, 배치 규칙을 다시 손댈 땐 반드시 함께 본다.

### 적 마릿수는 CSV에 없다 — 역산한다
`WaveTable.GetSpawnCount(wave, enemy, basePower)`가
`목표전력 / 적1기전력`으로 계산한다(최소 1). 마릿수를 CSV에 따로 적으면 `PowerCoef`와 이중 소스가 되어
한쪽만 고쳤을 때 밸런스가 조용히 어긋난다.

### 적 스프라이트 (2026-08-30-1 연결)
`ResUtil.Load<Sprite>(enemy.SpritePath)`를 **루프 밖에서 1회만** 읽어 넘긴다 —
같은 종류를 여러 마리 세우는데 마리마다 읽으면 같은 파일을 마릿수만큼 조회하게 된다.

★ 로드에 실패하면 [[BattleUnit]]이 `m_SymbolImage.enabled = (_sprite != null)`로 Image를 끄고,
`UnitTemplate` 루트엔 Image가 없어 **화면에 HP 바만 뜬다.** 조용히 사라지므로 실패 시 로그를 남긴다.
파일명 규칙은 `Image/InGame/Enemy/enemy_*`.

### 승패 판정
| 조건 | 결과 |
|---|---|
| 살아있는 적 0 | `Victory` |
| 살아있는 아군 0 | `Defeat` — 남은 적이 그대로 본거지로 가므로 그 웨이브는 진 것으로 끝낸다 |
| 그 외 | `Running` |

적이 `m_HomeLineX`를 넘으면 그 적은 사라지고 `m_HomeHit`이 1 오른다.
★ 아직 `homeHit`을 `RunData.TakeHomeDamage()`로 넘기는 호출부가 없다 — 웨이브 종료 처리가 붙을 때 연결한다.

### 공개 API
| 멤버 | 설명 |
|---|---|
| `Begin(judgeResult, grid, spritePool, wave)` | 양쪽 유닛 세우기 |
| `Tick(deltaTime)` | 전 유닛 진행 + 본거지 선 검사 + 승패 검사 |
| `Clear()` | 유닛 파괴 및 상태 초기화 |
| `result` / `homeHit` / `isRunning` | 읽기 |

### 검증 상태 — Codex QA 통과 A~G (2026-08-30)
`BattleStartButton.onClick.Invoke()`(프로덕션 진입점)로 시작해 확인:
아군 우측·적 좌측 이동 / HP 감소와 사망 비활성화 / 6.00s에 `Running → Defeat` /
적 마릿수 `0.85 × 6 ÷ 1 = 5.1 → 5` 일치 / 레인 교차 교전 0건.

### 남은 위험
- 36웨이브 테이블이 승인될 때 12연차 플레이어 전력을 ×18.0(프리즘)으로 잡았으나, 실제 판정 소환은
  3성 ×7.0에서 막힌다. 밸런스 재산정 필요(`UNFINISHED.md` 기록).

## 2026-08-31-0 — 타깃을 레인 밖까지 (가까운 상대 추적)

### 증상
"전투할 때 일직선으로만 가고 가까운 상대를 안 찾는다"(사용자, 2026-08-31).

### 원인
`FindTarget`이 **같은 레인만** 봤고(`other.lane != _unit.lane` → skip),
`BattleUnit.Tick`이 **x축으로만** 움직였다. 그래서 한 레인이 비면 그쪽 적이
아무 저항 없이 본거지까지 걸어갔고, 옆 레인 아군은 손을 못 댔다.

### 수정
- `FindTarget` — 레인 제한 제거, **2D 제곱거리**로 가장 가까운 반대편.
  x만 재면 다른 레인의 바로 위 적이 "가깝다"고 잡혀 엉뚱하게 달려간다
- `BattleUnit.Tick` — 목표 방향으로 `toTarget.normalized` 이동.
  **사거리 판정도 2D로** 바꿨다 — x만 재면 바로 위 칸의 적을 "사거리 안"으로 오판해 제자리에서 허공을 친다
- 목표가 없으면(반대편 전멸) 기존처럼 진격 방향으로만 간다. 적이 본거지 선을 넘는 경로가 살아 있어야 한다
- `RefreshDepthOrder()` — y로 그리기 순서 재정렬. **순서가 실제로 바뀐 프레임에만** 적용한다
  (매 프레임 `SetSiblingIndex`는 캔버스를 다시 그리게 한다)

★ 이게 없으면 유닛이 레인을 넘는 순간 스폰 당시 순서가 그대로 남아 **뒤 유닛이 앞을 덮는다.**

### 부수 효과 — 윷의 구조적 약점 완화
윷은 평균 2.63기만 소환해 레인이 자주 빈다(`UNFINISHED` 2026-08-31-0의 기록된 위험).
레인 제약이 사라지면 적은 수의 강한 유닛이 옆 레인까지 커버한다.

### 검증 — Codex QA 통과 (2026-08-31, Play Mode 실측)
- 아군 `(0,0)` / 적 `(500,104)` — 아군 y `0 → 11.00 → 21.99 → 32.99`, 1.90초 교전
- archer(사거리 324px): 같은 y 300px 즉시 공격 / 다른 y 360.56px 공격 안 하고 4.86px 접근
- 그리기 순서 y `130`→sibling 0, `40`→1, `-60`→3
- 1연차 1웨이브 `Running → Victory` 8.68초, **타임아웃 30회 중 0회**, 콘솔 에러 0

### ⚠️ 밸런스가 움직였다
적 grunt 5기 기준, 아군 전부 1등급 균등 배치, 각 10회:
| 아군 수 | 승률 | 평균 시간 |
|---|---|---|
| 3기 | **0%** | 7.42초 |
| 6기 | **100%** | 8.68초 |
| 9기 | 100% | 6.42초 |

경계가 3~6기 사이에 있고 급격하다. 평균 소환이 6기 근처라 1연차는 대체로 이기지만
**소환이 적게 나온 스핀은 확정 패배**다.
★ 이 표는 **윷에 적용하면 안 된다** — 윷은 2.63기를 2·3성 위주로 내는데 이 측정은 전부 1등급이다.
등급 섞인 측정은 아직 없다.


---

## 2026-09-13 — 복귀 지휘 (`ReturnSurvivorsToHome` · `isReturning`)

### 추가

`ReturnSurvivorsToHome(float _speedScale)` — 살아남은 **아군**을 원래 칸으로 돌려보내고
**실제로 움직이기 시작한 수**를 돌려준다. 0이면 호출부가 대기 없이 정리로 넘어간다
(전멸했거나 이미 다들 제자리라는 뜻).

`isReturning` — 아직 걸어가는 유닛이 하나라도 있는가. 복귀 종료 판정용.

### 적은 돌려보내지 않는다

살아남은 적은 다음 웨이브에 새로 생성되고 이 화면의 적 오브젝트는 곧 `Clear()`로 사라진다.
**걸어 돌아갈 "자기 자리"라는 개념이 적에게는 없다** — `SpawnEnemies`는 등장선에서 밀어 넣을 뿐이다.

### 죽은 유닛은 복귀하지 않는다 (결정)

`isAlive == false`로 걸러진다. 명부에는 살아 있으므로 다음 웨이브에 다시 서고,
**복귀가 끝난 뒤 `m_Field.Show()`가 전장 9칸을 전부 그리므로 그때 다시 나타난다.**

회색으로 되살아나 걸어오게 하는 안(설계안의 대안)은 택하지 않았다 —
"죽었는데 왜 살아 있나"로 읽히고, 회색 표현에 아트가 필요하다.
