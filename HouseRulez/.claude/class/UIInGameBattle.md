...
## 2026-08-31-0 — 타깃을 레인 밖까지 (가까운 상대 추적)

### 증상
"전투할 때 일직선으로만 가고 가까운 상대를 안 찾는다"(사용자, 2026-08-31).

### 원인
`FindTarget`이 **같은 레인만** 봤고(`other.lane != _unit.lane` → skip),
`BattleUnit.Tick`이 **x축으로만** 움직였다. 그래서 한 레인이 비면 그쪽 적이
아무 저항 없이 본거지까지 걸어갔고, 옆 레인 아군은 손을 못 댔다.

### 수정
- `FindTarget` — 레인 제한 제거, **2D 제곱거리**로 가장 가까운 반대편.
  x만 재면 다른 레인의 바로 위 적이 "가깝다"고 잡혀 엉뚱하게 달려간다
- `BattleUnit.Tick` — 목표 방향으로 `toTarget.normalized` 이동.
  **사거리 판정도 2D로** 바꿨다 — x만 재면 바로 위 칸의 적을 "사거리 안"으로 오판해 제자리에서 허공을 친다
- 목표가 없으면(반대편 전멸) 기존처럼 진격 방향으로만 간다. 적이 본거지 선을 넘는 경로가 살아 있어야 한다
- `RefreshDepthOrder()` — y로 그리기 순서 재정렬. **순서가 실제로 바뀐 프레임에만** 적용한다
  (매 프레임 `SetSiblingIndex`는 캔버스를 다시 그리게 한다)

★ 이게 없으면 유닛이 레인을 넘는 순간 스폰 당시 순서가 그대로 남아 **뒤 유닛이 앞을 덮는다.**

### 부수 효과 — 윷의 구조적 약점 완화
윷은 평균 2.63기만 소환해 레인이 자주 빈다(`UNFINISHED` 2026-08-31-0의 기록된 위험).
레인 제약이 사라지면 적은 수의 강한 유닛이 옆 레인까지 커버한다.

### 검증 — Codex QA 통과 (2026-08-31, Play Mode 실측)
- 아군 `(0,0)` / 적 `(500,104)` — 아군 y `0 → 11.00 → 21.99 → 32.99`, 1.90초 교전
- archer(사거리 324px): 같은 y 300px 즉시 공격 / 다른 y 360.56px 공격 안 하고 4.86px 접근
- 그리기 순서 y `130`→sibling 0, `40`→1, `-60`→3
- 1연차 1웨이브 `Running → Victory` 8.68초, **타임아웃 30회 중 0회**, 콘솔 에러 0

### ⚠️ 밸런스가 움직였다
적 grunt 5기 기준, 아군 전부 1등급 균등 배치, 각 10회:
| 아군 수 | 승률 | 평균 시간 |
|---|---|---|
| 3기 | **0%** | 7.42초 |
| 6기 | **100%** | 8.68초 |
...
경계가 3~6기 사이에 있고 급격하다. 평균 소환이 6기 근처라 1연차는 대체로 이기지만
**소환이 적게 나온 스핀은 확정 패배**다.
★ 이 표는 **윷에 적용하면 안 된다** — 윷은 2.63기를 2·3성 위주로 내는데 이 측정은 전부 1등급이다.
등급 섞인 측정은 아직 없다.
