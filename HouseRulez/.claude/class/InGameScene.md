...
## 2026-08-31-1 — 판정 이름 배너

릴이 멈추는 순간 [[UIInGameBanner]]로 `PatternName`을 화면 중앙에 크게 띄운다.
요약 패널은 접혀 있어서 안 열면 안 보이는데, 그러면 "왜 이만큼 소환됐지"를 배울 기회가
매 스핀 그냥 지나간다.

**`Power > 0`일 때만 띄운다.** 전력 0인 스핀이 화면을 덮을 이유가 없다.

무료 스핀 배너와 같은 컴포넌트지만 시간이 갈린다 — 판정 이름은 릴 정지 직후,
무료 스핀은 `m_SummonDelay`(0.9초) 뒤다. 겹쳐도 `Show()`가 이전 시퀀스를 죽이고 다시 시작한다.
