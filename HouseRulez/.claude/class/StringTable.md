...
## 2026-08-27-1 — `ReelSpin` 키 추가

`StringTable.csv` Id 25. 인게임 SPIN 버튼의 하드코딩을 걷어내면서 추가했다.

| Key | Kr | En | Cn | Jp |
|---|---|---|---|---|
| `ReelSpin` | SPIN | SPIN | SPIN | SPIN |

**네 언어 값이 모두 같은 건 의도다.** 목업에서 이 버튼만 영문 표기라 그대로 옮겼다 —
번역 누락이 아니다. 상세 근거는 [[InGameScene]] 2026-08-27-1 참고.

이로써 인게임 씬의 텍스트 중 키 없이 남은 것은 골드 액수(`GoldValue`)뿐이고, 그건 런타임에 숫자만
덮어쓰는 표시라 위 "다음에 키를 추가할 때" 2번대로 키를 만들지 않는 게 맞다.

---

## 2026-09-09-0 — `SettingsClose` 키 추가 (설정 닫기 버튼)

`StringTable.csv` Id 62. 설정 팝업의 닫기 버튼이 **어느 언어에서든 `"닫기"` 한글로 고정**이던
결함을 고치면서 추가했다. 상세는 [[UISetting]] 2026-09-09-1.

| Key | Kr | En | Cn | Jp |
|---|---|---|---|---|
| `SettingsClose` | 닫기 | Close | 关闭 | 閉じる |

- 동일 문구의 공용 키가 이미 있는지 먼저 검색했고 **없었다** — `Common*` 접두 키 자체가 0건이라
  이 테이블엔 아직 화면 무관 공용 키라는 개념이 없다. 그래서 기존 `Settings*` 관례를 따랐다
- **Id는 끝에 이어 붙였다**(61 다음). 조회는 `GetTemplate`이 `record.Key == _key`로 하므로
  Id 순서나 그룹핑에 의미가 없다. `Settings*` 블록(27~34) 사이에 끼워 넣으려면 이후 행을
  전부 재번호해야 해서 이득 없이 위험만 크다
- 파일은 **BOM 없음 · LF · 마지막 개행 있음** — 그 형식을 유지해 append 했다

### ✅ 검증
Play Mode에서 4개 언어 전부 정상 표시, `□` 0건을 확인했다(`qa-quick-check` 위임).
`关闭`·`閉じる`는 이 프로젝트에서 처음 렌더되는 문자열이라 글리프 유무가 미지수였는데,
각각 Vonwaon·PixelMplus가 커버했다.

**키 누락의 부정 경로도 함께 확인했다** — `GetTemplate`은 키를 못 찾으면 `Logger.Error`를 남기고
**키 문자열 자체를 반환**하므로, CSV가 안 읽혔으면 화면에 `SettingsClose`가 그대로 뜬다.
그렇게 뜨지 않는 것을 확인해 CSV 로드 성공을 증명했다.
